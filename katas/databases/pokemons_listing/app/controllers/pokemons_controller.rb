class PokemonsController < ApplicationController
  before_action :set_pokemon,
                only: [:show, :edit, :update, :destroy, :remove_category]
  ...
  def remove_category
    ...
    @pokemon.categories.delete(category)

    respond_to do |format|
      format.html { redirect_to @pokemon, notice: 'Catgory was successfully removed.' }
      format.json { render :show, status: :ok, location: @pokemon }
    end
  end
  ...
  private
    ...
    def pokemon_params
      params
        .require(:pokemon)
        .permit(:name, :number, :evolution_id)
    end
end
